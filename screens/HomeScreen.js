...
const HomeScreen = ({ route, navigation }) => {
  const [rosetas, setRosetas] = useState([]);

  // Función para obtener las rosetas
  const fetchRosetas = async () => {
    try {
      ...
      setRosetas(response.data); // Asignar las rosetas al estado
    } catch (error) {
  ...
  // Usar useFocusEffect para actualizar al volver a la pantalla
  useFocusEffect(
    React.useCallback(() => {
      fetchRosetas();
    }, [])
    ...
    <SafeAreaView style={styles.container}>
      ...
      {rosetas.length > 0 ? (
        <FlatList
          ...
          keyExtractor={(item) => item.id_roseta.toString()}
      ...
      <View style={styles.menu}>
        ...
        <TouchableOpacity style={styles.menuButton} onPress={() => navigation.navigate('Devices')}>
          <View style={styles.iconContainer}>
